 ...
 * 
 * @author Jim Teresco (Modified by Saif Ullah & Nate Anderson)
 * @version Spring 2022 
 */

public class Hierarchy extends MouseAdapter implements Runnable {
	...
	protected DraggableShape dragging;
	...
	public void run() {
        ...
        //objects constructed and added to the shapes list
		...
		for (int i = 0; i < count; i++) {
			...
			//shapes.add(new DraggableShape(point, r.nextInt(), r.nextBoolean(), size, color));
}
	...
	public static void main(String args[]) {
		...
		javax.swing.SwingUtilities.invokeLater(new Hierarchy(count));
	}
}
...
abstract class DraggableShape {
	...
	public DraggableShape(int size,
	...
	public DraggableShape(Point point, int i, boolean nextBoolean, int size2, Color color2) {
	}
...
abstract class Circle extends DraggableShape{
	...
	public boolean contains(Point p){
        Point center = new Point(upperLeft.x + size, upperLeft.y + size);
		return center.distance(p) <= size;
	}

}

abstract class Square extends DraggableShape{

	public Square(int size, Point upperLeft, Color color) {
		super(size, upperLeft, color);
	}
...
class FramedCircle extends Circle{
...
class FramedSquare extends Square{
